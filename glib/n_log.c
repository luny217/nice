...
static int n_log_level = N_LOG_INFO;
static int flags;
...
{
    [N_LOG_PANIC  / 8] =  52 << 16 | 196 << 8 | 0x41,
    [N_LOG_FATAL  / 8] = 208 <<  8 | 0x41,
    [N_LOG_ERROR  / 8] = 196 <<  8 | 0x11,
    [N_LOG_WARNING / 8] = 226 <<  8 | 0x03,
    [N_LOG_INFO   / 8] = 253 <<  8 | 0x09,
    [N_LOG_VERBOSE / 8] =  40 <<  8 | 0x02,
    [N_LOG_DEBUG  / 8] =  34 <<  8 | 0x02,
...
void n_log(void * avcl, int level, const char * fmt, ...)
{
    AVClass * avc = avcl ? *(AVClass **) avcl : NULL;
    va_list vl;
    va_start(vl, fmt);
    if (avc && avc->version >= (50 << 16 | 15 << 8 | 2) &&
            avc->log_level_offset_offset && level >= N_LOG_FATAL)
        level += *(int *)(((uint8_t *) avcl) + avc->log_level_offset_offset);
    av_vlog(avcl, level, fmt, vl);
    va_end(vl);
}

void n_vlog(void * avcl, int level, const char * fmt, va_list vl)
{
    void (*log_callback)(void *, int, const char *, va_list) = n_log_callback;
    if (log_callback)
        log_callback(avcl, level, fmt, vl);
}

int n_log_get_level(void)
{
    return n_log_level;
}
...
void n_log_set_flags(int arg)
{
    flags = arg;
}

int n_log_get_flags(void)
{
    return flags;
}

void n_log_set_callback(void (*callback)(void *, int, const char *, va_list))
{
    n_log_callback = callback;
}

static void missing_feature_sample(int sample, void * avc, const char * msg, va_list argument_list)
{
    n_vlog(avc, N_LOG_WARNING, msg, argument_list);
    n_log(avc, N_LOG_WARNING, " is not implemented. Update your FFmpeg "
          "version to the newest one from Git. If the problem still "
          "occurs, it means that your file has a feature which has not "
          "been implemented.\n");
    if (sample)
        n_log(avc, N_LOG_WARNING, "If you want to help, upload a sample "
